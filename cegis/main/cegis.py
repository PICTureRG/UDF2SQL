...
def pbe(udf, examples, last_loc=0, last_depth=0):
    ...
    spec1 = f"{DIR}/spec.tyrell.jinja2.small"
    spec2 = f"{DIR}/spec.tyrell.jinja2"
    synth1 = None
    synth2 = None
    ...
    for loc in range(0, maxloc+1):
        ...
        for depth in range(loc+2, loc+3):
            if depth < last_depth:
                continue
            last_depth = 0
            LOG.info(f"trying small grammar loc={loc}, depth={depth}")
            if not synth1:
                synth1 = get_synth(spec1)
            try:
                sql = synth1.synthesize(loc=loc, depth=depth)
                if sql:
                    STATS.pbe.append((loc, depth, synth1.num_attempts, synth1.time_elapsed, synth1.enumerator_init_time))
                    return (sql, loc, depth)
            except:
                synth1.num_attempts = 0
                synth1.time_elapsed = 0
                synth1.enumerator_init_time = 0

            if loc == 0:
                STATS.pbe.append((loc, depth, synth1.num_attempts, synth1.time_elapsed, synth1.enumerator_init_time))
                continue
            
            LOG.info(f"trying large grammar loc={loc}, depth={depth}")
            if not synth2:
                synth2 = get_synth(spec2)
            sql = synth2.synthesize(loc=loc, depth=depth)
            STATS.pbe.append((loc, depth, synth1.num_attempts+synth2.num_attempts, synth1.time_elapsed+synth2.time_elapsed, synth1.enumerator_init_time+synth2.enumerator_init_time))
            if sql:
                return (sql, loc, depth)
