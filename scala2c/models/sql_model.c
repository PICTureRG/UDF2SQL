...
String SQL_string_array_join(AString a, String delim) {
    if (a->size == 0) return constString("");
    ...
    size = 0;
    memcpy(buf+size, a->arr[0]->buf, a->arr[0]->size);
    size += a->arr[0]->size;
    for (int i=1; i<a->size; i++) {
        memcpy(buf+size, a->arr[i]->buf, a->arr[i]->size);
        size += a->arr[i]->size;
        memcpy(buf+size, delim->buf, delim->size);
        size += delim->size;
    }
